...
export function ScrollStory({ lines, className = '' }: ScrollStoryProps) {
  ...
  return <div ref={stage} className={`story-reveal ${className}`} style={{ height: `${lines.length * 48}vh` } as CSSProperties}>
    {lines.map((line, index) => <p className="story-line" style={{ top: `${index * 48}vh` } as CSSProperties} key={`${index}-${line}`}>{line}</p>)}
  </div>
}
